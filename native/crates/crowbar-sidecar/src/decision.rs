...
//! platform types so the policies are unit-testable.
//!
//! Moved verbatim from `desktop/src-tauri/src/sidecar/supervisor.rs` (S0.3);
//! renamed from `supervisor` to `decision` because this crate's own root now
//! carries the name "sidecar supervisor," and `crowbar_sidecar::supervisor`
//! would have said the same word twice for no reason. Nothing in this file
//! changed except that name and the one test below that no longer needs a
//! desktop-local fd-count serialisation guard — see its own comment.
...
/// # Errors
///
/// Any `io::Error` from the metadata read or rename, except a missing file —
/// that is not an error here, since there is then nothing to rotate.
...
mod tests {
    ...
    fn cycle_was_suspended_flags_app_nap_and_system_sleep_gaps() {
        ...
        assert!(
            cycle_was_suspended(Duration::from_hours(1), interval, probe),
    ...
    fn restart_budget_allows_up_to_max_within_window() {
        let mut b = RestartBudget::new(3, Duration::from_mins(10));
        let now = Instant::now();
        assert!(b.allow(now));
        assert!(b.allow(now));
        assert!(b.allow(now));
    ...
    fn restart_budget_frees_slots_as_the_window_slides() {
        let mut b = RestartBudget::new(2, Duration::from_mins(10));
    ...
    fn rotate_if_needed_moves_full_log_aside() {
        // Unlike the desktop original, this crate's own test suite has no
        // other test that counts this process's open file descriptors, so
        // there is nothing here for opening a log file to race against — the
        // `crate::test_support::fd_tests_blocking()` guard that call sat
        // behind in `desktop/src-tauri` does not need a counterpart here.
