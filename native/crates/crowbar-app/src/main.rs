...
//!
//! Everything here so far is a measurement:
//!
//! * **the matrix surfaces** ([`row_surface`] over [`surfaces`]) — one surface,
//!   drawn in one cell of the §8.3 matrix, chosen by the command line. This is
//!   what a bare `cargo run -p crowbar-app` opens. A surface registers by being
//!   a file in `src/surfaces/`; nothing dispatches on which one it is.
