//! The Phase 1 reference row: `--surface git-status-row`, the default surface.
//!
//! These were `row_layout.rs`'s own top-level tests, written before there was a
//! second surface to share the harness with. They moved here unchanged when the
//! file was split, so that *every* surface is a file and adding one is adding
//! one — the harness they call is still `super`'s, and still the only copy.
