...
|---|---|---|---|
| 1 | strict-parity anchors converge across the §8.3 matrix | ❌ **not met, and further off than this row used to say** | **Two gaps, not one.** *(a)* **Denominator**: §5.1 covers `components/ui` **and** `components/layout`; the second is at **1 of 29** and was never counted — so "43 surfaces" measured against half the definition. *(b)* **References**: for the surfaces that do exist, 258/258 cells are *runnable* but references exist for **one** cell of most. Viewport axis inert on all of them, theme axis vacuous on 10, and v1.14 shows a reference is only comparable if the app is driven back into its original configuration — which nothing records. **2026-08-04, the layout tier moved: 10 of 22 now hold a PASS** (`sidebar-project-header`, `context-pill`, `workspace-branch-icon`, `project-home-row`, `sidebar-carousel`, `sidebar-toast-overlay`, `-fallback`, `repo-icon-popover`, `repo-section`, `workspace-tree-item`) — but see the `workspace-tree` finding: a hardcoded `px(344.0)` survived **every** recorded run because all of them drove exactly `--width 344`. **A single-cell reference cannot tell a value that tracks its input from a constant that happens to equal it** — which is this row's own gap, observed for the first time on a *port* constant rather than a reference absence. **This row's *references exist for one cell of most* is the THIRD BLIND SPOT at scale** — rediscovered independently on `button`, whose reference is nine icon-only buttons, leaving its whole content-sized path untested by construction. An uncaptured cell produces **no delta and no warning**; the surface simply passes. So the gap is not only *fewer cells measured* — it is that **nothing in a green run distinguishes a covered cell from an absent one.** |
...
| `resizable-needs-a-taller-display.md` | **user decision** *(or a bigger display, or one dev capability)* | **2026-08-04: now gates THREE surfaces, not one** — `resizable`, `repo-import-dialog`, `detach-holder-modal`. Root cause identified: `core:window:allow-set-size` is absent from the app's allow-list, so `window.set_size` is refused and the bridge's own resize returns `success: true` while doing nothing. A **dev-only** capability would clear all three; adding it to the shipping `capabilities/default.json` is the decision. ✅ qualifies |
...
| `locked-screen-blocks-every-capture.md` | ✅ **RESOLVED 2026-08-04 (later the same day)** — the screen was unlocked and the whole stranded backlog was cleared: 5 re-verdicts taken, 4 now PASS. The hazard stands as a hazard; it is no longer blocking anything. *(original classification below)* ⛔ **RECLASSIFIED 2026-08-04 — open, and only the user can clear it** | ~~environmental note, not a work item~~. I wrote that on the grounds that the lock *was real when raised and can recur; captures work today*. **It recurred**, and is now stranding four merged-but-unverified surfaces. A hazard that has fired is not a note. It also *does* satisfy §17.7's wording after all — unlocking the machine is a thing only the user can do — so **§17.7 is met, but by a worse route than I recorded**: not *no open work*, but *the open work is the user's*. |
