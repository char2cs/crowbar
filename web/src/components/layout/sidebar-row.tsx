...
const ROW =
  'flex items-center h-9 px-2 mx-1.5 gap-2 rounded-lg my-0.5 cursor-pointer overflow-hidden select-none'
...
export function ChatRow({ title, age, active, onClick, onDelete }: ChatRowProps) {
  return (
    <div
      className={cn(ROW, 'group', active ? 'bg-accent' : 'hover:bg-accent/50')}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={
    ...
    >
      <div
        aria-hidden="true"
        className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-md border border-border bg-card text-[11px]"
      >
        💬
      </div>
      <span
      ...
      <span
        className={cn(
          'flex-shrink-0 text-[11px] text-muted-foreground/50',
...
export function NewRow({ label, onClick }: NewRowProps) {
  return (
    <div
      className={cn(ROW, 'hover:bg-accent/50')}
      onClick={onClick}
      role="button"
      tabIndex={0}
      onKeyDown={
