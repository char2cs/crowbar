  ...
  return (
    ...
    >
      ...
      <SliderPrimitive.Control
        ...
        data-oracle-id="slider"
        data-slot="slider-control"
      >
        {/*
          `data-oracle-id="slider-track"` marks the HOST, not its visible
          pill: the pill is painted by `::before` (native/oracle/ANCHORS.md
          §3), and `before:inset-x-0.5` means it is inset 2px on each
          horizontal edge — not `inset:0`, so the extractor's pseudo-backed
          shortcut (which assumes `inset:0`) reports the host's own box here,
          not the pill's. See `native/mapping/slider.md`.
        */}
        ...
        >
          <SliderPrimitive.Indicator
            ...
            data-oracle-id="slider-indicator"
          ...
          {Array.from({ length: _values.length }, (_, index) => (
            <SliderPrimitive.Thumb
              ...
              data-oracle-id={index === 0 ? 'slider-thumb' : undefined}
