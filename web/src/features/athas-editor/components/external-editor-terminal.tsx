import { terminalWrite, terminalResize, terminalListen } from '@/lib/crowbar-bridge';
...
export const ExternalEditorTerminal = ({
  ...
  const initializeTerminal = useCallback(() => {
    ...
    terminal.onData((data) => {
      terminalWrite(terminalConnectionId, data).catch((e) => {
    ...
    terminal.onKey(({ domEvent }) => {
      ...
      if ((e.metaKey && e.key === "Backspace") || (e.ctrlKey && e.key === "u")) {
        e.preventDefault();
        terminalWrite(terminalConnectionId, "\u0015").catch((e) => {
    ...
    // Set up terminal data listener via crowbar-bridge
    const setupListeners = () => {
      const unlisten = terminalListen(terminalConnectionId, (data: string) => {
        terminal.write(data);
      });

      // Store cleanup function
      (terminal as unknown as { _cleanupListeners: () => void })._cleanupListeners = unlisten;
    };
      ...
      resizeRafRef.current = requestAnimationFrame(() => {
        if (fitAddonRef.current && terminalRef.current) {
          ...
          if (xtermRef.current) {
            ...
            terminalResize(terminalConnectionId, rows, cols).catch((e) => {
