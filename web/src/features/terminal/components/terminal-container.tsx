  ...
  const closeTerminal = useCallback(
    (terminalId: string, options: CloseTerminalOptions = {}) => {
      ...
      if (session?.connectionId) {
        void terminalClose(session.connectionId).catch((error) => {
    ...
    (terminalId: string, name: string) => {
      ...
      if (wsStore) {
        wsStore.setState((state) => ({
          ...state,
          buffers: state.buffers.map((buf) =>
            buf.type === "terminal" && (buf as { sessionId?: string }).sessionId === terminalId
              ? { ...buf, name: trimmedName }
              : buf,
          ),
        }));
      }
  ...
  useEffect(() => {
    const handleTerminalReady = (event: Event) => {
      ...
      if (pendingCommand && connectionId) {
        ...
        setTimeout(() => {
          terminalWrite(connectionId, pendingCommand).catch(() => {});
