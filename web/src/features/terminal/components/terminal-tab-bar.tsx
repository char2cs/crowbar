...
// Tauri file dialog/fs replaced with browser download for web mode
const save = async (_opts: unknown): Promise<string | null> => null
const writeTextFile = async (_path: string, content: string): Promise<void> => {
  // Browser download fallback
  const blob = new Blob([content], { type: 'text/plain' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a'); a.href = url; a.download = 'terminal.txt'; a.click()
...
import { Dropdown, MenuItemsList, type MenuItem } from "@/components/ui/dropdown";
