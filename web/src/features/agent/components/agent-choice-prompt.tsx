...
/** Everything this card remembers, stamped with the prompt it belongs to.
 *
 *  The stamp is load-bearing. This component is mounted inside a chat pane that is
 *  RETAINED across chat selection, so a re-render can hand it a different prompt
 *  without unmounting it — and picks left over from the previous one would be
 *  submitted against option ids that prompt never offered. Which is the partial
 *  answer this whole feature exists to make impossible, arrived at from the other
 *  direction. */
type CardState = {
  choiceId: string
        ...
        <>
          {questions.length > 0 ? (
            <ChoiceQuestions
              ...
              onSend={() => void send(flatPicks(questions, card.byQuestion))}
            />
          ) : (
            <ChoiceButtons
              options={controls}
 ...
 *  pick. It stays disabled until every question has one, which is what makes a
 *  partial answer impossible from here — the shape that stranded a live agent. */
function ChoiceQuestions({
  ...
  const answeredAll = questions.every((question) => (picked[question.id] ?? []).length > 0)
  return (
