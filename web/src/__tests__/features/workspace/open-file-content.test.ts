...
describe('openFileContent', () => {
  it('fetches the workspace-scoped content route and opens a buffer', async () => {
    ...
    await openFileContent('ws-1', 'web/vite.config.ts', { openContent } as never, { preview: false })
    expect(apiFetch).toHaveBeenCalledWith('/v0/workspaces/ws-1/files/content?path=web%2Fvite.config.ts')
  ...
  it('decodes base64-encoded content', async () => {
    ;(apiFetch as ReturnType<typeof vi.fn>).mockResolvedValue({ content: btoa('binary'), encoding: 'base64' })
    const openContent = vi.fn()
