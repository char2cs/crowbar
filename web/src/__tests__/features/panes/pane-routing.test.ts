...
import { getPaneScopeForPaneId, resolveWritablePaneForBuffer } from "@/features/panes/utils/pane-routing";
...
describe("pane routing", () => {
  ...
  it("routes new buffers to the most recent unlocked pane in the same tree", () => {
    const activePane = makeGroup(ROOT_PANE_ID, { bufferIds: ["buffer-a"], locked: true });
    const firstFallback = makeGroup("pane-b", { bufferIds: ["buffer-b"], activeBufferId: "buffer-b" });
    const mostRecentFallback = makeGroup("pane-c", { bufferIds: ["buffer-c"], activeBufferId: "buffer-c" });
  ...
  it("returns null when every pane in the active tree is locked", () => {
    ...
    const rootLayout: LayoutNode = createSplit("horizontal", createLeaf(ROOT_PANE_ID), createLeaf("pane-b"));
    const bottomLayout = createLeaf(BOTTOM_PANE_ID);
  ...
  it("keeps routing scope within the active pane tree", () => {
    ...
    const bottomActivePane = makeGroup("bottom-active", { bufferIds: ["terminal-a"], activeBufferId: "terminal-a" });
    const bottomFallbackPane = makeGroup("bottom-fallback", { bufferIds: ["terminal-b"], activeBufferId: "terminal-b" });
    const rootLayout = createLeaf(ROOT_PANE_ID);
    const bottomLayout = createSplit("horizontal", createLeaf("bottom-active"), createLeaf("bottom-fallback"));
    const panes = {
