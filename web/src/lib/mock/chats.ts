...
const INITIAL_CHATS: MockChat[] = [
  {
    ...
    messages: [
      ...
      },
      {
        ...
        role: 'assistant',
        content:
          'The three products share a user identity layer, so a shared auth service is the right foundation. crowbar handles agent orchestration, quiver.core is the shared library, and quiver.desktop consumes both.',
        authorName: 'Claude',
        authorInitials: '✦',
        modelName: 'Sonnet 4.6',
        timestamp: '2h ago · 6.1s',
        toolCalls: 2,
        durationSec: 6.1,
      },
    ],
...
const store = new Map<string, MockChat>(INITIAL_CHATS.map((c) => [c.id, c]))
