...
const MESSAGE_POOLS: Array<Array<[string | null, boolean, string]>> = [
  [
    [null, false, 'This function is O(n²) in the worst case because of the nested loop. With the data sizes we\'re expecting this will be noticeably slow.'],
    ['Aria', true, 'Confirmed — if `items` is large this is a real perf issue. Build a `Map<id, item>` in a single pass first, then the inner lookup becomes O(1). Overall drops to O(n).'],
    [null, false, 'Refactored to use a Map. Benchmarks show 40× speedup on 10k items.'],
    ['Aria', true, 'The Map is rebuilt on every call. If this is hot path, memoize it or use `useMemo`.'],
    [null, false, 'Added `useMemo` with items array as dependency.'],
  ],
  [
    ['Morgan', false, 'Nit: the variable name `data` is too generic. Given the context it should be `userProfile`.'],
    [null, false, 'Renamed throughout.'],
    ['Aria', true, '`profilePayload` implies it\'s being sent somewhere. If it\'s received, `profileRecord` reads more naturally.'],
  ...
  [
    ...
    ['Aria', true, '`null` is correct here — the backend explicitly returns `null` when absent (not omitting the key). Worth a comment explaining the distinction.'],
    [null, false, 'Added comment: `// null = backend explicitly absent, not unset`.'],
    ['Morgan', false, 'Could we encode this at the type level? A `type ExplicitNull<T> = T | null` with JSDoc.'],
    ['Aria', true, 'Good idea but overkill for now. If this pattern appears in 3+ places, introduce the type alias.'],
  ],
  [
    ['Aria', true, 'This `useEffect` has `user` in the dep array but `user.id` is what\'s actually used inside. The effect re-runs on any identity change even if id didn\'t change.'],
    [null, false, 'Fixed — changed `[user]` to `[user.id]`.'],
    ['Aria', true, 'If `user` can be null here, `user.id` would throw. Add a null guard: `[user?.id]`.'],
  ...
  [
    ...
    ['Aria', true, 'Silent failures make debugging very hard. At minimum log in dev. Better: propagate to UI via error state.'],
    ['Morgan', false, 'We have a `reportError()` util that sends to Sentry in prod and logs in dev.'],
    [null, false, 'Added `reportError(err)` in the catch. Also added error state that shows a toast.'],
    ['Aria', true, 'Make sure the toast message is user-friendly and doesn\'t expose internal details like stack traces.'],
    [null, false, "The toast just says \"Something went wrong. Please try again.\""],
  ],
  [
    ['Aria', true, 'This component re-renders every time the parent re-renders because the callback is defined inline. Wrap it in `useCallback`.'],
    ...
    ['Morgan', false, '`dispatch` from Zustand is guaranteed stable — defined once in store creation.'],
  ],
  [
    [null, false, 'The regex will fail for email addresses with subdomains (e.g. `user@mail.company.co.uk`).'],
    ['Aria', true, 'Correct catch. Use `/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/` — it allows any non-whitespace/@ with at least one dot after @.'],
  ...
  [
    ['Morgan', false, 'This API call doesn\'t handle timeouts. On a slow connection the user will see a spinner forever.'],
    ['Aria', true, 'Add an `AbortController` with a timeout. Store the timeout id and clear it in `finally`.'],
...
const EXTREME_FILES = [
  'src/lib/query/query-layer.ts', 'src/lib/query/cache-manager.ts',
  'src/api/auth/session.ts', 'src/features/dashboard/components/ActivityFeed.tsx',
  'src/features/editor/stores/buffer-slice.ts', 'src/lib/ws/channel.ts',
  'src/middleware/rate-limit.ts', 'src/lib/crypto/token.ts',
  'src/features/git/components/diff-viewer.tsx', 'src/api/v2/workspaces.ts',
  'src/features/billing/stripe-webhook.ts', 'src/lib/db/connection-pool.ts',
]
...
function genThreads(wsId: string, count: number): ReviewThread[] {
  return Array.from({ length: count }, (_, i) => {
    ...
    const messages = pool.map(([author, isAgent, body], j) => msg(
      `msg-${wsId}-${i}-${j}`, author, isAgent, body,
...
function makeMassiveDiff(wsId: string, seed: number) {
  ...
  const files = Array.from({ length: fileCount }, (_, fi) => {
    ...
    return {
      ...
      is_new: fi < 3, is_deleted: false, is_renamed: false,
      additions, deletions,
      lines: [
...
const EXTREME_MSGS = [
  ...
  'fix: race condition in session token refresh', 'perf: reduce bundle size by 34%',
...
const AUTHORS_EX = ['Mateo Urrutia', 'Claude Agent', 'Alice Chen', 'Bob Rodriguez', 'Dependabot[bot]', 'Sofia Andersson']
...
function genChats(wsId: string, count: number): BranchReviewChat[] {
  const titles = [
    ...
    'Cache invalidation design', 'Error handling patterns', 'TypeScript migration', 'WebSocket reconnect logic',
  ]
  return Array.from({ length: count }, (_, i) => ({
    id: `chat-${wsId}-${i}`,
