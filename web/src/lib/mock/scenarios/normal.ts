...
function makeLine(type: GitDiffLine['line_type'], content: string, old?: number, newL?: number): GitDiffLine {
...
const DIFFS: Record<string, any> = {
  'rb-onboarding': {
    ...
    totalFiles: 4, totalAdditions: 340, totalDeletions: 12,
    files: [
      {
...
const THREADS: Record<string, ReviewThread[]> = {
  'rb-onboarding': [
    {
      id: 'rbt-1', filePath: 'src/features/onboarding/OnboardingWizard.tsx', lineNumber: 12,
      ...
      messages: [
        ...
        msg('rbm-1-2', 'Aria', true, "Good UX consideration. Add a `useEffect` that writes `step` to localStorage on change, and reads it back as the `useState` initial value. Clear it on the complete step.", '2026-06-01T10:01:00Z'),
        msg('rbm-1-3', null, false, 'Done — added in the next commit.', '2026-06-01T10:05:00Z'),
      ],
    },
    {
      id: 'rbt-2', filePath: 'src/api/onboarding.ts', lineNumber: 10,
      side: 'right', isResolved: false,
      messages: [
        msg('rbm-2-1', 'Aria', true, "The `inviteEmails: string[]` field should be validated server-side too. A malformed email slipping through would cause a confusing 422 from the API.", '2026-06-01T10:15:00Z'),
        msg('rbm-2-2', null, false, 'Agreed — adding Zod validation to the API route handler.', '2026-06-01T10:20:00Z'),
        msg('rbm-2-3', 'Aria', true, 'Also consider rate-limiting this endpoint — someone could enumerate whether emails are registered.', '2026-06-01T10:21:00Z'),
      ],
    },
    {
      id: 'rbt-3', filePath: 'src/router.tsx', lineNumber: 18,
      side: 'right', isResolved: true,
      messages: [
        msg('rbm-3-1', null, false, 'Should `/onboarding` be protected by an auth guard?', '2026-06-01T10:30:00Z'),
        msg('rbm-3-2', 'Aria', true, "Protect it with auth but add an `isOnboardingComplete` flag to the user profile. Redirect to `/dashboard` if they're already onboarded.", '2026-06-01T10:31:00Z'),
      ],
    },
  ],
  'rb-fix': [
    {
      id: 'rbt-4', filePath: 'src/features/auth/SignupForm.tsx', lineNumber: 34,
      side: 'right', isResolved: false,
      messages: [
        msg('rbm-4-1', 'Aria', true, "The error message 'Signup failed' is user-friendly but consider distinguishing network errors from server errors — a toast for network, inline for validation.", '2026-06-01T11:00:00Z'),
        msg('rbm-4-2', null, false, 'Fair point, will handle in a follow-up — this PR is just the email validation fix.', '2026-06-01T11:02:00Z'),
      ],
    },
  ],
}
...
const COMMIT_MSGS = [
  'feat(onboarding): add multi-step wizard', 'feat(auth): implement signup with email verification',
  'fix(signup): validate email before submission', 'chore: add vitest configuration',
  'feat(dashboard): add activity feed component', 'fix(api): handle rate limit errors gracefully',
  'refactor(auth): extract token refresh logic', 'feat(profile): add avatar upload',
  'chore: upgrade React to v19', 'docs: update API integration guide',
  'feat(billing): add Stripe webhook handler', 'fix(router): handle 404 on unknown routes',
  'test: add auth flow integration tests', 'feat(settings): add notification preferences',
