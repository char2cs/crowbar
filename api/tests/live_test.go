...
// TestLive_GitSnapshotThenLiveUpdate proves the watcher chain on the Git topic:
// the snapshot arrives on connect, then a real on-disk edit (re-written on a
// ticker so the test never races the lazily started watcher goroutine) fans out
// as a live GitStatus frame carrying the dirty file.
...
// TestLive_FileChangeEvent proves the watcher chain on the Files topic: a real
// edit fans out as a FileChangeEvent scoped to the subscribing workspace.
func TestLive_FileChangeEvent(t *testing.T) {
	h := newHarness(t)
	imported := importProject(t, h)

	conn := h.dial("/v0/ws/files?wsId=" + imported.workspaceID)
...
// Workspaces topic: the current workspace row is delivered immediately on connect,
// before any live event, so the sidebar renders live on first connect.
func TestLive_WorkspaceSnapshotOnConnect(t *testing.T) {
	h := newHarness(t)
	imported := importProject(t, h)

	conn := h.dial("/v0/ws/workspaces?projectId=" + imported.projectID)
	got := readUntil(t, conn, func(m map[string]any) bool {
...
// TestLive_GitWsIdScopingIsolatesWorkspaces proves wsId scoping: a Git subscriber
// for workspace A receives A's snapshot but never workspace B's, even after B is
// edited on disk. Two real repos are imported so both watchers are driveable.
func TestLive_GitWsIdScopingIsolatesWorkspaces(t *testing.T) {
	h := newHarness(t)
	a := importProject(t, h)
	b := importProject(t, h)

	conn := h.dial("/v0/ws/git?wsId=" + a.workspaceID)
	...
	bConn := h.dial("/v0/ws/git?wsId=" + b.workspaceID)
