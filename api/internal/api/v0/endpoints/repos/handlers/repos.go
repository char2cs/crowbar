...
type WorkspaceRemover interface {
	DeleteRepoWorkspaces(ctx context.Context, repoID string, repoPath string) error
}
...
func (h *Handlers) removeRepoWorkspaces(
	...
	if err := h.wsRemover.DeleteRepoWorkspaces(ctx, repoID, repoPath); err != nil {
		slog.ErrorContext(ctx, "delete repo: remove workspaces", "repo", repoID, "err", err)
	}
	h.purgeRemainingWorkspaceRows(ctx, repoID)
}
