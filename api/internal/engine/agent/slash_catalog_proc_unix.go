...
// isolateProcessGroup makes the probe its own process-group leader (pgid == pid)
// so cancellation can signal the whole group. Without it a provider CLI that
// forks its own helpers leaves those helpers running after the probe is killed:
// exec.CommandContext only ever signals the direct child.
