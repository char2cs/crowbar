...
// New builds the chat read-model store, registers the projection that keeps it in
// sync with the aggregate, then reconciles every read-model row from the
// authoritative event store for each id in aggregateIDs.
//
...
// model, without reconcile-on-open a chat would be missing or stale forever after
// a mid-projection crash, while Get (which reads the event store) stays correct —
// a permanent divergence. The reconcile is best-effort: a failed row never blocks
// opening the store, and the next write self-corrects it. The chat aggregate is
// global (one store + one asynx for every chat), so the repo enumerates the event
// store's aggregate ids and passes them here.
func New(
	ctx context.Context,
	db *gormdb.DB,
...
func (s *storeService) reconcile(
	...
	for _, id := range aggregateIDs {
		if err := s.reconcileOne(ctx, ax, id); err != nil {
			slog.WarnContext(ctx, "chat store: read-model reconcile failed; self-corrects on next write",
