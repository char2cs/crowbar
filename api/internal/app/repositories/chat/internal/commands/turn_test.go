...
// Conformance assertions: verify each command implements asynxModels.Command[domain.Chat].
var (
	_ asynxModels.Command[domain.Chat] = commands.StartTurn{}
	_ asynxModels.Command[domain.Chat] = commands.StopTurn{}
)
...
func TestStartTurn_EmitEvent_InputUnmutated(t *testing.T) {
	chat := &domain.Chat{ID: "c1", Working: false, CurrentTurnStarted: nil}
...
func TestStopTurn_EmitEvent_InputUnmutated(t *testing.T) {
	...
	chat := &domain.Chat{ID: "c1", Working: true, CurrentTurnStarted: &turnStarted}
	beforeWorking := chat.Working
	beforeTurnStarted := chat.CurrentTurnStarted
