...
func TestApply_AnEventWithNoDeltaIsANoOp(t *testing.T) {
	...
	require.NoError(t, p.Apply(context.Background(), domain.ChatActivity{ChatID: "c1"}))
...
func TestApply_TolerossMalformedDeltas(t *testing.T) {
	...
	for _, delta := range testCases {
		d := delta
		assert.NoError(t, p.Apply(ctx, domain.ChatActivity{ChatID: "c1", Last: &d}), d.Kind)
	}
}
...
func TestApply_ReportsAStorageFailureRatherThanPanicking(t *testing.T) {
	...
	for _, delta := range deltas {
		d := delta
		assert.Error(t, p.Apply(ctx, domain.ChatActivity{ChatID: "c1", Last: &d}), d.Kind)
	}
