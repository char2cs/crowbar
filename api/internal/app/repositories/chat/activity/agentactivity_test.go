...
func (f fixture) turn(t *testing.T, id, role, text string, at time.Time) {
	t.Helper()
	require.NoError(t, f.repo.AppendTurn(f.ctx, activity.TurnInput{
...
func TestValidation_IsSurfacedAndNeverRetried(t *testing.T) {
	...
	err := f.repo.AppendTurn(f.ctx, activity.TurnInput{ChatID: chat, TurnID: "t", Role: "narrator"})
...
func TestReadModel_IsRebuiltByReplayAndTheRebuildIsIdempotent(t *testing.T) {
	...
	require.NoError(t, f.repo.OpenChoice(f.ctx, activity.ChoiceInput{
