...
func TestEveryCommand_NamesItsAggregateAndEvent(t *testing.T) {
	cmds := []asynxModels.Command[domain.AgentActivity]{
...
func TestSnapshotPolicy_FollowsEventFrequency(t *testing.T) {
	...
	hot := []asynxModels.Command[domain.AgentActivity]{
		commands.InvokeTool{},
...
func TestRegression_ACloseSideEventNeverConjuresATurn(t *testing.T) {
	...
	testCases := []struct {
		name string
		emit func(*domain.AgentActivity) domain.AgentActivity
	}{
		{"subagent stop", func(s *domain.AgentActivity) domain.AgentActivity {
			return commands.StopSubagent{ChatID: chat, SubagentID: "a1", Now: now}.EmitEvent(s)
		}},
		{"tool completion", func(s *domain.AgentActivity) domain.AgentActivity {
			return commands.CompleteTool{ChatID: chat, ToolID: "t9", Now: now}.EmitEvent(s)
		}},
		{"interruption resolution", func(s *domain.AgentActivity) domain.AgentActivity {
