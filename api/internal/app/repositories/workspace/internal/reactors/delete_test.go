...
func TestRegisterDeleteReactor_GatedPurge_RemovesWorktreeAndForgets(t *testing.T) {
	...
	gate := drain.New()
	purger := NewPurger(ax, noDrop, forgetDependents, func(path string) error { rmCh <- path; return nil })
	...
	assert.Equal(t, "/wt/w1/worktree", <-rmCh, "the purge removes the tombstone's own WorktreePath")
