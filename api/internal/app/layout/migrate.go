...
type Workspace struct {
	...
	AdoptedPath string
	// ChatIDs are the agent chats this workspace owns, needed only by the
	// PRE-LEAF shape: its chats dir is filepath.Dir(worktreePath)/chats, which
	// resolves to <slug>/chats — outside the directory being moved, and SHARED
	// with every other pre-leaf workspace of the repo. Nothing in a path says
	// which conversation belongs to whom, so the mapping has to be supplied.
	//
	// A leaf-shaped workspace needs none of this: its chats dir is a sibling of
	// its worktree inside the root, and moves with it.
	ChatIDs []string
}
...
type movePlan struct {
	...
	root     string
	worktree string
	// oldChatsDir is the shared chats directory a PRE-LEAF workspace's ledgers
	// live in, and is empty for the leaf shape, whose chats are inside the root
	// being moved and need no separate handling.
	oldChatsDir string
}
...
func planMove(home string, ws Workspace) (movePlan, error) {
	...
	if bare {
		...
		p.dest = p.worktree
		// ChatsDir(worktreePath) is filepath.Dir(worktreePath)/chats — for this
		// shape the SLUG directory's chats, which the move above does not touch.
		p.oldChatsDir = filepath.Join(filepath.Dir(ws.WorktreePath), "chats")
	}
	if p.oldRoot == p.root {
		return movePlan{}, errSkip
	}
...
func applyMove(
	...
	// So did a pre-leaf workspace's conversation ledgers.
	carryChats(ws, p)
...
// carryChats moves a pre-leaf workspace's conversation ledgers from the shared
// <slug>/chats into its own root, where ChatsDir will look for them once the
// record names the new worktree.
//
// Only the chats this workspace OWNS move: the directory is shared with every
// other pre-leaf workspace of the repo, and taking the whole thing would hand
// one workspace another's history and leave the other with none.
//
// Best-effort, like every other step whose failure the alias absorbs: a ledger
// left behind is history that needs recovering by hand, which is bad, but a
// migration that fails outright and leaves the workspace half-moved is worse.
func carryChats(ws Workspace, p movePlan) {
	if p.oldChatsDir == "" || len(ws.ChatIDs) == 0 {
		return
	}
	newChatsDir := filepath.Join(p.root, "chats")
	for _, chatID := range ws.ChatIDs {
		if chatID == "" {
			continue
		}
		src := filepath.Join(p.oldChatsDir, chatID)
		if _, err := os.Stat(src); err != nil {
			continue // no ledger on disk for this chat
		}
		dst := filepath.Join(newChatsDir, chatID)
		if _, err := os.Stat(dst); err == nil {
			continue // already carried by an earlier, interrupted run
		}
		//nolint:gosec // G301: as elsewhere in this layout.
		if err := os.MkdirAll(newChatsDir, 0o755); err != nil {
			slog.Warn("layout migration: could not create the chats dir",
				"ws", ws.ID, "dir", newChatsDir, "err", err)
			return
		}
		if err := os.Rename(src, dst); err != nil {
			slog.Warn("layout migration: could not carry a chat ledger",
				"ws", ws.ID, "chat", chatID, "from", src, "err", err)
			continue
		}
		slog.Info("layout migration: carried a chat ledger",
			"ws", ws.ID, "chat", chatID, "to", dst)
	}
}
