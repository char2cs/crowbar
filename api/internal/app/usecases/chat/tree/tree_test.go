...
func newUsecase(
	t *testing.T,
) (*mocks.AgentChatFolderStore, *mocks.AgentChatPlacements, tree.Usecase) {
	t.Helper()
	folders := mocks.NewAgentChatFolderStore()
	chats := mocks.NewAgentChatPlacements()
	return folders, chats, tree.New(folders, chats, chats)
}
...
func TestCreate_AppendsAtTheEndOfTheSiblingSpace(t *testing.T) {
	...
	created, shifted, err := uc.Create(ctx, tree.CreateInput{
...
func TestCreate_TrimsAndRefusesABlankName(t *testing.T) {
	...
	_, _, err = uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "   "})
...
func TestRename_TrimsAndRefusesABlankName(t *testing.T) {
	folders, _, uc := newUsecase(t)
	ctx := context.Background()
	created, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "old"})
	require.NoError(t, err)
...
func TestMove_DensifiesBothLevels(t *testing.T) {
	...
	seedChat(chats, "c1", 1)
	seedChat(chats, "c2", 2)
	moved, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "spikes"})
	require.NoError(t, err)

	placed, shifted, err := uc.Move(ctx, workspaceID, moved.ID, tree.MoveInput{
		ParentID: name("c1"),
	})
	require.NoError(t, err)
...
func TestMove_WithNothingRequestedKeepsThePlacement(t *testing.T) {
	...
	placed, _, err := uc.Move(ctx, workspaceID, created.ID, tree.MoveInput{})
	require.NoError(t, err)
	assert.Equal(t, "c1", placed.ParentID)
...
func TestMove_ReordersWithinALevelAndReportsTheCollateral(t *testing.T) {
	_, _, uc := newUsecase(t)
	ctx := context.Background()
	first, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "a"})
	require.NoError(t, err)
	second, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "b"})
	require.NoError(t, err)

	placed, shifted, err := uc.Move(ctx, workspaceID, second.ID, tree.MoveInput{Order: index(0)})
	require.NoError(t, err)
	assert.Equal(t, 0, placed.Order)
...
func TestMove_RefusesAFolderOntoItself(t *testing.T) {
	_, _, uc := newUsecase(t)
	ctx := context.Background()
	created, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "spikes"})
	require.NoError(t, err)

	_, _, err = uc.Move(ctx, workspaceID, created.ID, tree.MoveInput{ParentID: name(created.ID)})
...
func TestMove_RefusesAMoveIntoItsOwnSubtree(t *testing.T) {
	_, _, uc := newUsecase(t)
	ctx := context.Background()
	outer, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "outer"})
	require.NoError(t, err)
	...
	_, _, err = uc.Move(ctx, workspaceID, outer.ID, tree.MoveInput{ParentID: name(inner.ID)})
	assert.ErrorIs(t, err, tree.ErrCycle)
}
...
func TestMove_RefusesAFolderInAnotherWorkspace(t *testing.T) {
	...
	_, _, err := uc.Move(context.Background(), workspaceID, "f-other", tree.MoveInput{})
	assert.ErrorIs(t, err, apperr.ErrNotFound)
}
...
func TestMove_SurfacesASnapshotFailure(t *testing.T) {
	...
	_, _, err = uc.Move(ctx, workspaceID, created.ID, tree.MoveInput{})
	assert.ErrorContains(t, err, "chats down")
}
...
func TestMove_RefusesAParentThatDoesNotExist(t *testing.T) {
	...
	_, _, err = uc.Move(ctx, workspaceID, created.ID, tree.MoveInput{ParentID: name("nowhere")})
	assert.ErrorIs(t, err, apperr.ErrNotFound)
}
...
func TestMove_SurfacesASaveFailure(t *testing.T) {
	folders, _, uc := newUsecase(t)
	ctx := context.Background()
	_, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "a"})
	require.NoError(t, err)
	second, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "b"})
	require.NoError(t, err)
	folders.SaveErr = errors.New("disk full")

	_, _, err = uc.Move(ctx, workspaceID, second.ID, tree.MoveInput{Order: index(0)})
	assert.ErrorContains(t, err, "disk full")
}
...
func TestDeleteChat_SurfacesADensifyWriteFailure(t *testing.T) {
	...
	ctx := context.Background()
	seedChat(chats, "c1", 1)
	_, _, err := uc.Create(ctx, tree.CreateInput{WorkspaceID: workspaceID, Name: "spikes"})
	require.NoError(t, err)
	folders.SaveErr = errors.New("disk full")
