...
func (u *runnerUsecase) Compact(ctx context.Context, chatID string) error {
	chat, err := u.chats.GetChat(ctx, chatID)
	if err != nil {
		return fmt.Errorf("agent: compact: %w", err)
	}

	providerID, err := u.chat.chatProviderID(ctx, chatID)
	if err != nil {
		return fmt.Errorf("agent: compact: %w", err)
	}
	...
	agent, err := u.agents.Get(ctx, crowbarHome, providerID)
	if err != nil {
	...
	if _, err := u.SubmitPrompt(ctx, chatID, text, uuid.NewString()); err != nil {
		return fmt.Errorf("agent: compact: %w", err)
	}
	return nil
}
