// Package turnstate holds the two process-local views of whether a chat is busy:
// the registry of turns currently in flight, and the mirror of the authoritative
...
func (w *Turns) changedLocked(chatID string) chan struct{} {
	ch := w.changed[chatID]
	if ch == nil {
		ch = make(chan struct{})
		w.changed[chatID] = ch
...
func (w *Turns) signalLocked(chatID string) {
	if chatID == "" {
		return
	}
...
func (w *Turns) Watch(chatID string) (bool, <-chan struct{}) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func NewWork() *Work {
	return &Work{states: map[string]*workState{}}
}

func (w *Work) stateLocked(chatID string) *workState {
	state := w.states[chatID]
	if state == nil {
		state = &workState{changed: make(chan struct{})}
...
func (w *Work) Set(chatID string, working bool) {
	if chatID == "" {
		return
	}
	w.mu.Lock()
...
func (w *Work) Observe(chatID string) (working, known bool, changed <-chan struct{}) {
	w.mu.Lock()
	defer w.mu.Unlock()
