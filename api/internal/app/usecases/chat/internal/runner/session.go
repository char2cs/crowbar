...
func (rs *Runners) HandleSessionStart(
	ctx context.Context,
	...
	knownChatID, err := rs.runnerStore.ChatForSession(ctx, runner.WorkspaceID, ev.SessionID)
	known := err == nil
	...
	case engineagents.MoveBind:
		...
		ok, err := rs.requirePlacement(ctx, runner, runner.CurrentChatID)
		if err != nil || !ok {
			return err
		}
	...
	case engineagents.MoveToKnown:
		...
		ok, err := rs.requirePlacement(ctx, runner, d.ChatID)
		if err != nil || !ok {
			return err
		}
