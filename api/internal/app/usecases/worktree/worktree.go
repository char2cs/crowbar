...
// locally forking from the parent — from origin's tip of the parent when the
// parent is on origin (see parentStartPoint), else from the local parent ref —
// and the fork point is the start SHA reported by WorktreeAddBranch. The two
// paths have DIFFERENT fork-point semantics: the checkout path's fork point is
// the remote tip (so later merge/reparent math diffs against what the remote
// already contains), while the create path's fork point is exactly where the
// new branch diverged from its parent (origin's tip of that parent when reachable).
...
// from. It returns origin's fresh tip of the parent when the parent exists on the
// `origin` remote, and the local parent ref otherwise.
//
// The child must fork from origin's LATEST parent tip, not a stale local copy —
// the field bug this guards against branched enhancement/performance off a develop
// that was 10 commits behind origin/develop. But we must refresh origin's tip
// WITHOUT moving the local parent ref: `git fetch origin <parent>:<parent>`
// (FastForwardBranch) is REFUSED by git whenever <parent> is checked out in ANY
// worktree, and in Crowbar every protected branch — the default branch included —
// is permanently checked out in its own locked managed worktree, so that
// fast-forward deterministically failed for the most common parent and creation
// then silently forked from the stale local tip. FetchRef updates only
// refs/remotes/origin/<parent> (`git fetch origin <parent>`), which git always
// allows regardless of checkouts; we then fork the child from the resolved
// origin/<parent> SHA. Advancing the local parent ref (and its locked worktree) is
// OriginSyncManager's job, never ours — so we never pass --update-head-ok.
//
...
// origin/<branch> tip. FastForwardBranch (which MOVES the local branch ref) is
// correct here — unlike parentStartPoint, which must never move the parent ref —
