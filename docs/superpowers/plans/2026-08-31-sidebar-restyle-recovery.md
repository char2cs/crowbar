...
## Task 6: Production readiness — findings-dependent, scope confirmed after Task 1

**Files:** determined by Task 1's report. At minimum:
...
- Possibly: `desktop/src-tauri/src/lib.rs` (vibrancy fallback) and `web/src/features/agent/components/agent-terminal-wait-banner.tsx` (composer overlap) — **only if Task 1's live check actually confirms these are broken.** Do not "fix" either speculatively; the recovery spec explicitly flags both as plausible-but-unconfirmed.

**Interfaces:** none fixed in advance — this task's implementer reads `task-1-findings.md` first and scopes their own diff against it, then reports back what they found and fixed.
...
- [ ] **Step 3: If and only if Task 1 confirmed the vibrancy failure live** (not just "the screenshot looked photographic" — an actual confirmed `apply_vibrancy` failure or missing fallback), fix `desktop/src-tauri/src/lib.rs` so a failed vibrancy attach falls back to a solid, non-transparent background rather than leaving `transparent: true` showing the raw desktop. Write this task's own regression coverage in whatever this repo's existing pattern is for Rust/Tauri-side tests (check for existing tests near the vibrancy code first).
- [ ] **Step 4: If and only if Task 1 confirmed the trust-banner overlaps the composer live**, reposition `agent-terminal-wait-banner.tsx` (or reserve space for it in `agent-chat-pane.tsx`'s blank-chat layout) so it never sits on top of the input. This is a positioning fix, not a removal — the banner's underlying behavior is correct and pre-existing (recovery spec §2.3).
