...
Whether a worktree is currently active is derived state: query AgentRuns with `status: running` that reference it. No status field is stored on the Worktree itself — there is no lock preventing multiple tasks from being assigned to the same worktree.
