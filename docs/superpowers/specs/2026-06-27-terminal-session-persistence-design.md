...
- **Detached** — the last client WS closed (workspace switch). PTY keeps running; output continues into the ring. `lastActiveAt` recorded. A session with a **running foreground process is never auto-suspended** — it stays Detached until re-attach, with the sole exception of last-resort force-suspend under the global hard ceiling (see *Session Limits*).
- **Suspended** — a Detached, zero-client session whose PTY has been intentionally torn down: normally an **idle** session (no running foreground process) selected by its workspace's soft limit or the global ceiling; or, in the bounded last-resort case, a still-running session force-suspended to enforce the global ceiling. The PTY is killed *intentionally* (distinct from termination — see issue-driven note below), scrollback flushed to disk, CWD captured. On re-attach a fresh shell spawns **under the same session id** in the saved CWD and the scrollback replays.
...
- **Global hard ceiling:** max total sessions (default 100) **and** max total ring bytes (default 256MB), tracked O(1) on the single global registry map. When exceeded, suspend the oldest-`lastActiveAt` **idle** Detached session across all workspaces. If no idle Detached candidate exists and we are still over budget, the last resort is a **force-suspend** of the oldest-`lastActiveAt` Detached session — even one with a running process: it follows the *suspend* path (set `suspending`, kill the process, flush `.buf`+row, suppress the `ended` frame, keep the registry entry), writing a one-line notice into its scrollback first, so it returns as **Suspended** on next attach — it is **not** a terminal `Kill`/`Exited` (which deletes persistence). This is the one bounded exception to "suspension never targets a running session"; it exists so the hard ceiling is enforceable. (Running processes already cannot survive app close/restart — see *What This Does NOT Solve*.)
...
3. The last-client-removal detach transition inside the blocking `Attach` (≈ terminal.go:265), plus `Suspend`/`Restore` and the reap path, push a lifecycle frame with the correct status via the existing broadcaster. (There is no separate engine-level `Detach` method — see *Concurrency*; the `detached` frame is pushed at that transition site.) Detach pushes `detached`; Suspend pushes `suspended` (never `ended`); Restore pushes `active`/`detached`; reap pushes `ended` (+ `exitCode`).
...
- `internal/session/session.go`: retain `shell`/`profileId`/mutable `cwd`; add `state` + `IsLive()` (`ptmx != nil`) + `NewPlaceholder`; `AttachedCount()`; `isIdleLocked()`/`IsIdle()` (tcgetpgrp); `suspending` flag; `flushMu`; OSC 7 scan in the pump; hold `s.mu` across `ring.Write`+`fanOut`; `Kill` takes `s.mu` only around `ptmx.Close`/`Process.Kill` then releases before `shutdown()`; flush goroutine bound to `s.Done()`. **Placeholder (`ptmx == nil`) safety:** `Kill`, `shutdown`, and `isIdleLocked` must short-circuit when `ptmx == nil` — a placeholder has no process/fd, so `Kill` just drops the registry entry (the row/`.buf` are handled by the caller) and does no PTY teardown or `cmd.Wait()`; `isIdleLocked` reports a placeholder as not-idle/not-applicable without an ioctl. A placeholder **never** starts a `reapOnDone` goroutine (its `<-s.Done()` would block forever).
- `terminal.go`: refactor `Create` → private `spawn(id, …)`; make `Attach` restore-aware (`Restore` when `!IsLive`); add `Suspend` (re-verify under lock, set `suspending`, kill the process, flush + `metaStore.Save state=suspended`, suppress the `ended` frame), `Restore` (re-register same id, load `.buf`, reset state); auto-suspend sweep (10s, soft limit + global ceiling, evict idle by `lastActiveAt`, **force-suspend** last-resort — still the suspend path, never a terminal Kill); make `reapOnDone` honor `suspending`; on a **real** exit/Kill (not suspend, not force-suspend) do `metaStore.Delete` + `.buf` delete; treat a `!IsLive()` placeholder as already-suspended during eviction/`Shutdown` (no PTY teardown, no idle ioctl); capture `cmd.Wait()` exit code for the `ended` frame.
